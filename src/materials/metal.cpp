...
void MetalMaterial::ComputeScatteringFunctions(SurfaceInteraction *si,
    ...
    auto frMf = std::make_shared<FresnelConductor>(1., eta->Evaluate(*si), k->Evaluate(*si));
    auto distrib = std::make_shared<TrowbridgeReitzDistribution>(uRough, vRough);

    si->bsdf->Add(std::make_shared<MicrofacetReflection>(1., distrib, frMf));
}

constexpr int CopperSamples = 56;
constexpr float CopperWavelengths[CopperSamples] = {
...
constexpr float CopperN[CopperSamples] = {
...
constexpr float CopperK[CopperSamples] = {
...
//     static SampledSpectrum copperN = SampledSpectrum::FromSampled(CopperWavelengths, CopperN, CopperSamples);
//     std::shared_ptr<Texture<SampledSpectrum> eta = std::make_shared<Texture<SampledSpectrum>>(copperN);

//     static SampledSpectrum copperK = SampledSpectrum::FromSampled(CopperWavelengths, CopperK, CopperSamples);
//     std::shared_ptr<Texture<SampledSpectrum>> k = std::make_shared<Texture<SampledSpectrum>>(copperK);

//     std::shared_ptr<Texture<float>> roughness = std::make_shared<Texture<float>>(0.01f);

//     std::shared_ptr<Texture<float>> uRoughness = std::make_shared<Texture<float>>(0.0f);

//     std::shared_ptr<Texture<float>> vRoughness = std::make_shared<Texture<float>>(0.0f);

//     std::shared_ptr<Texture<float>> bumpMap = std::make_shared<Texture<float>>(0.0f);

//     bool remapRoughness = true;

//     return std::make_shared<MetalMaterial>(eta, k, roughness, uRoughness, vRoughness, bumpMap, remapRoughness);
// }
