...
#include <PBRender/core/primes.h>
#include <PBRender/core/rng.h>
#include <PBRender/core/vecmath.h>
#include <PBRender/samplers/sobolmatrices.h>
...
class DigitPermutation {
  public:
    ...
    DigitPermutation(int base, uint32_t seed) : base(base) {
        ...
        permutations.clear();
        permutations.resize(nDigits * base);
...
inline uint64_t SobolIntervalToIndex(uint32_t log2Scale,
                                                  uint64_t sampleIndex, Point2i p);

inline float BlueNoiseSample(Point2i p, int instance);
...
inline float ScrambledRadicalInverse(int baseIndex, uint64_t a,
...
inline uint32_t MultiplyGenerator(std::vector<uint32_t> C, uint32_t a) {
    uint32_t v = 0;
