...
// TODO: check `BSDF` implementation with PBRT-v3
class BSDF {
public:
    // BSDF Public Methods
    BSDF() = default;
    BSDF(Normal3f ns, Vector3f dpdus, std::shared_ptr<BxDF> bxdf)
    ...
    Optional<BSDFSample> Sample_f(
        ...
        Optional<BSDFSample> bs = bxdf->Sample_f(wo, u, u2, mode, sampleFlags);
        if (bs)
            // DCHECK_GE(bs->pdf, 0);
            assert(bs->pdf > 0);
    ...
    //     Optional<BSDFSample> bs = specificBxDF->Sample_f(wo, u, u2, mode, sampleFlags);
    //     if (!bs || !bs->f || bs->pdf == 0 || bs->wi.z == 0)
    //         return {};
    //     // DCHECK_GT(bs->pdf, 0);
